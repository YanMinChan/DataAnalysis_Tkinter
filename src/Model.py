...
docs_views_country = df.loc[
    (df["subject_doc_id"] == doc_id) & (df["event_type"] == "impression")
]
...
for i in docs_views_country.index:
    docs_views_country.loc[i, "continent"] = pc.country_alpha2_to_continent_code(
        docs_views_country.loc[i, "visitor_country"]
    )
...
df.loc[df["visitor_useragent"].str.lower().str.contains("mozilla"), "browser"] = (
    "firefox"
)
...
def viewer(doc_uuid):
    viewers = df.loc[
        (df["subject_doc_id"] == doc_uuid) & (df["event_type"] == "impression")
    ]
...
def documents(viewer_uuid):
    document = df.loc[
        (df["visitor_uuid"] == viewer_uuid) & (df["event_type"] == "impression")
    ]
...
def also_likes(doc_uuid, user_uuid, sort):
    ...
    res = [(key, len(list(group))) for key, group in res_iter if key not in to_remove]
    res = sort(res)
    return res


def sorting(docs):
    return [z[0] for z in sorted(docs, key=lambda y: y[1], reverse=True)]
